...
Create original knowledge comics with flexible art style 脳 tone combinations.
...
Concrete tool names (`imagegen`, `image_generate`, `t2i-imagine`) above are examples 鈥?substitute the local equivalents under the same rule.

## Reference Images

Users may supply reference images to guide art style, palette, scene composition, or subject. This is **separate from** the auto-generated character sheet (Step 7.1) 鈥?both can coexist: user refs guide the look, the character sheet anchors recurring character identity.

**Intake**: Accept via `--ref <files...>` or when the user provides file paths / pastes images in conversation.
- File path(s) 鈫?copy to `refs/NN-ref-{slug}.{ext}` alongside the comic output
- Pasted image with no path 鈫?ask the user for the path (per the User Input Tools rule above), or extract style traits verbally as a text fallback
- No reference 鈫?skip this section
...
- If `usage: direct` AND the chosen backend accepts multiple reference images 鈫?pass both the character sheet (Step 7.2) and the user refs via the backend's ref parameter; compress images first per Step 7.1's guidance to avoid payload failures
- If the backend accepts only one ref 鈫?prefer the character sheet for pages with recurring characters; embed user-ref traits in the prompt body instead
- For `style`/`palette` usage 鈫?embed extracted traits in every page's prompt text (applies regardless of backend capability)
...
- **Presets** (5) with special rules beyond plain art+tone:
  ...
  | `four-panel` | minimalist + neutral + four-panel layout | 璧锋壙杞悎 structure, B&W + spot color, stick-figure characters |

  Full rules at `references/presets/<preset>.md` 鈥?load the file when a preset is picked.

- **Compatibility matrix** and **content-signal 鈫?preset** table live in [references/auto-selection.md](references/auto-selection.md). Read it before recommending combinations in Step 2.
...
4. Resolve `${BUN_X}` runtime: if `bun` installed 鈫?`bun`; if `npx` available 鈫?`npx -y bun`; else suggest installing bun
...
- [ ] Step 1: Setup & Analyze
  - [ ] 1.1 Preferences (EXTEND.md) 鉀?BLOCKING
    - [ ] Found 鈫?load preferences 鈫?continue
    - [ ] Not found 鈫?run first-time setup 鈫?MUST complete before other steps
  - [ ] 1.2 Analyze, 1.3 Check existing
- [ ] Step 2: Confirmation - Style & options 鈿狅笍 REQUIRED
- [ ] Step 3: Generate storyboard + characters
- [ ] Step 4: Review outline (conditional)
- [ ] Step 5: Generate prompts
- [ ] Step 6: Review prompts (conditional)
- [ ] Step 7: Generate images
  - [ ] 7.1 Generate character sheet (if needed) 鈫?characters/characters.png
...
```
Input 鈫?[Preferences] 鈹€鈹攢 Found 鈫?Continue
                       鈹?
                       鈹斺攢 Not found 鈫?First-Time Setup 鉀?BLOCKING
                                      鈹?
                                      鈹斺攢 Complete setup 鈫?Save EXTEND.md 鈫?Continue
                                                                              鈹?
        ...
        鈫?
Analyze 鈫?[Check Existing?] 鈫?[Confirm: Style + Reviews] 鈫?Storyboard 鈫?[Review?] 鈫?Prompts 鈫?[Review?] 鈫?Images 鈫?PDF 鈫?Complete
```
...
|------|--------|------------|
| 1.1 | Load EXTEND.md preferences 鉀?BLOCKING if not found | Config loaded |
...
**Pick a backend once per session** using the `## Image Generation Tools` rule at the top. If the backend is a repo skill (e.g., `t2i-imagine`), read its `SKILL.md` and use its documented interface rather than its scripts.

**7.1 Character sheet** 鈥?generate it (to `characters/characters.png`, aspect `4:3`) when the comic is multi-page with recurring characters. Skip for simple presets (e.g., four-panel minimalist) or single-page comics. Compress to JPEG before use-as-`--ref` (`sips -s format jpeg -s formatOptions 80 鈥 on macOS, `pngquant --quality=65-80 鈥 elsewhere) to avoid payload failures. The prompt file at `characters/characters.md` must exist before invoking the backend.

**7.2 Pages** 鈥?each page's prompt MUST already be at `prompts/NN-{cover|page}-[slug].md` before invoking the backend; the file is the reproducibility record. Strategy depends on the character sheet:
...
| Skipped | 鈥?| All descriptions inline in prompt |

**Backup rule**: existing `prompts/鈥d` and `鈥ng` files 鈫?rename with `-backup-YYYYMMDD-HHMMSS` suffix before regenerating. Aspect ratio from storyboard (default `3:4`; preset may override).

**`--ref` failure recovery**: compress sheet 鈫?retry 鈫?still fails 鈫?drop `--ref` and embed character descriptions in the prompt text.
...
### EXTEND.md Paths 鉀?BLOCKING

If EXTEND.md is not found, first-time setup is **blocking** 鈥?complete it before any content analysis or style/tone questions.
...
| Not found | 鉀?Run first-time setup ([references/config/first-time-setup.md](references/config/first-time-setup.md)) 鈫?save EXTEND.md 鈫?continue |
...
| Action | Steps |
|--------|-------|
| **Edit** | **Update prompt file FIRST** 鈫?`--regenerate N` 鈫?Regenerate PDF |
| **Add** | Create prompt at position 鈫?Generate with character ref 鈫?Renumber subsequent 鈫?Update storyboard 鈫?Regenerate PDF |
| **Delete** | Remove files 鈫?Renumber subsequent 鈫?Update storyboard 鈫?Regenerate PDF |
