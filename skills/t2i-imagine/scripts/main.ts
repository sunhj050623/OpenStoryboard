  ...
  for (const { current, legacy } of getExtendConfigPathPairs(cwd, home)) {
    ...
    for (const candidate of legacy) {
      ...
      try {
        ...
        // Ignore migration failures (e.g. read-only home mounts in containers).
      }
      break;
    }
  }
...
export async function loadExtendConfig(
  ...
  const paths = getExtendConfigPathPairs(cwd, home).flatMap(({ current, legacy }) => [current, ...legacy]);
