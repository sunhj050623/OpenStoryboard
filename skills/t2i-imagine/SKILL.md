...
Official API-based image generation. Supports OpenAI, Azure OpenAI, Google, OpenRouter, DashScope (闃块噷閫氫箟涓囪薄), Z.AI GLM-Image, MiniMax, Jimeng (鍗虫ⅵ), Seedream (璞嗗寘) and Replicate.
...
## Step 0: Load Preferences 鉀?BLOCKING

This step MUST complete before any image generation 鈥?generation is blocked until EXTEND.md exists.
...
Minimum working examples 鈥?see `references/usage-examples.md` for the full set including per-provider invocations and batch mode.
...
| `--model <id>`, `-m` | Model ID 鈥?see provider references for defaults and allowed values |
| `--ar <ratio>` | Aspect ratio (`16:9`, `1:1`, `4:3`, 鈥? |
| `--size <WxH>` | Explicit size (e.g., `1024x1024`) |
| `--quality normal\|2k` | Quality preset (default: `2k`) |
| `--imageSize 1K\|2K\|4K` | Image size for Google/OpenRouter (default: from quality) |
| `--imageApiDialect openai-native\|ratio-metadata` | OpenAI-compatible endpoint dialect 鈥?use `ratio-metadata` for gateways that expect aspect-ratio `size` plus `metadata.resolution` |
...
| `JIMENG_ACCESS_KEY_ID`, `JIMENG_SECRET_ACCESS_KEY` | Jimeng (鍗虫ⅵ) Volcengine credentials |
| `ARK_API_KEY` | Seedream (璞嗗寘) Volcengine ARK API key |
...
Priority (highest 鈫?lowest) applies to every provider:
...
1. `--ref` provided + no `--provider` 鈫?auto-select Google 鈫?OpenAI 鈫?Azure 鈫?OpenRouter 鈫?Replicate 鈫?Seedream 鈫?MiniMax (MiniMax's subject reference is more specialized toward character/portrait consistency)
2. `--provider` specified 鈫?use it (if `--ref`, must be google/openai/azure/openrouter/replicate/seedream/minimax)
3. Only one API key present 鈫?use that provider
4. Multiple keys 鈫?default priority: Google 鈫?OpenAI 鈫?Azure 鈫?OpenRouter 鈫?DashScope 鈫?Z.AI 鈫?MiniMax 鈫?Replicate 鈫?Jimeng 鈫?Seedream
...
- Replicate: behavior is model-specific 鈥?`google/nano-banana*` uses `aspect_ratio`, `bytedance/seedream-*` uses documented Replicate ratios, Wan 2.7 maps `--ar` to a concrete `size`
...
| Input is `outline.md` + `prompts/` (e.g. from `baoyu-article-illustrator`) | Batch 鈥?use `scripts/build-batch.ts` to assemble the payload | The outline + prompt files already contain everything needed |
...
- Missing API key 鈫?error with setup instructions
- Generation failure 鈫?auto-retry up to 3 attempts per image
- Invalid aspect ratio 鈫?warning, proceed with default
- Reference images with unsupported provider/model 鈫?error with fix hint
