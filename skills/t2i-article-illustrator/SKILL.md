...
Analyze articles, identify illustration positions, generate images with Type 脳 Style 脳 Palette consistency.
...
| **Palette** | Color scheme (optional) | macaron, warm, neon 鈥?overrides style's default colors |

Combine freely: `--type infographic --style vector-illustration --palette macaron`

Or use presets: `--preset edu-visual` 鈫?type + style + palette in one flag. See [Style Presets](references/style-presets.md).
...
| **Q3: Style** | [Recommended], minimal-flat, sci-fi, hand-drawn, editorial, scene, poster, Other 鈥?**skip if preset chosen** |
| Q4: Palette | Default (style colors), macaron, warm, neon 鈥?**skip if preset includes palette or preferred_palette set** |
| Q5: Language | When article language 鈮?EXTEND.md setting |
...
鉀?**BLOCKING: Prompt files MUST be saved before ANY image generation.** This is a hard requirement regardless of which backend is chosen 鈥?the prompt file is the reproducibility record.
