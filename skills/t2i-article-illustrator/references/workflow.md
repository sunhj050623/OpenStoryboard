...
|-------|------------------|------|
| File path | EXTEND.md `default_output_dir` (default: `imgs-subdir`). If not configured, confirm in 1.2. | 鈫?1.2 |
...
### Q1: Preset or Type 鈿狅笍 REQUIRED

Based on Step 2 content analysis, recommend a preset first (sets both type & style). Look up [style-presets.md](style-presets.md) "Content Type 鈫?Preset Recommendations" table.

- [Recommended preset] 鈥?[brief: type + style + why] (Recommended)
- [Alternative preset] 鈥?[brief]
- Or choose type manually: infographic / scene / flowchart / comparison / framework / timeline / mixed

**If user picks a preset 鈫?skip Q3** (type & style both resolved).
**If user picks a type 鈫?Q3 is REQUIRED.**
...
### Q3: Style 鈿狅笍 REQUIRED (skip if preset chosen in Q1)
...
- `macaron` 鈥?soft pastel blocks on warm cream
- `warm` 鈥?warm earth tones, no cool colors
- `neon` 鈥?vibrant neon on dark backgrounds
...
### Q5: Image Text Language 鈿狅笍 REQUIRED when article language 鈮?EXTEND.md `language`
...
6. **Prompt quality requirements** (all REQUIRED):
   ...
   - `LABELS`: Use **actual numbers, terms, metrics, quotes from the article** 鈥?NOT generic placeholders
...
**Verification** 鉀? Before proceeding to 5.2, confirm ALL prompt files exist:
```
Prompt Files:
- prompts/01-infographic-overview.md 鉁?
- prompts/02-infographic-distillation.md 鉁?
...
```
