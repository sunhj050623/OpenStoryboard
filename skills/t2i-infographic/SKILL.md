...
Two dimensions: **layout** (information structure) 脳 **style** (visual aesthetics). Freely combine any layout with any style.
...
- File path(s) 鈫?copy to `refs/NN-ref-{slug}.{ext}` alongside the output
...
- If `usage: direct` AND the chosen backend accepts reference images (e.g., `t2i-imagine` via `--ref`) 鈫?pass the file via the backend's ref parameter
- Otherwise 鈫?embed extracted `style`/`palette` traits in the prompt text
...
|--------------|--------|--------------------|----------------|--------------|
| 楂樺瘑搴︿俊鎭ぇ鍥?/ high-density-info | `dense-modules` | `morandi-journal`, `pop-laboratory`, `retro-pop-grid` | portrait | 鈥?|
| 淇℃伅鍥?/ infographic | `bento-grid` | `craft-handmade` | landscape | Minimalist: clean canvas, ample whitespace, no complex background textures. Simple cartoon elements and icons only. |
...
鈹溾攢鈹€ analysis.md
鈹溾攢鈹€ structured-content.md
鈹溾攢鈹€ prompts/infographic.md
鈹斺攢鈹€ infographic.png
```
...
- Preserve source data faithfully鈥攏o summarization or rephrasing (but **strip any credentials, API keys, tokens, or secrets** before including in outputs)
...
Check EXTEND.md in priority order 鈥?the first one found wins:
...
1. Save source content (file path or paste 鈫?`source.md`)
...
### Step 2: Generate Structured Content 鈫?`structured-content.md`
...
**3.2 Otherwise**, recommend 3-5 layout脳style combinations based on:
- Data structure 鈫?matching layout
- Content tone 鈫?matching style
...
|----------|----------|------|---------|
| 1 | **Combination** | Always | 3+ layout脳style combos with rationale |
| 2 | **Aspect** | Always | Named presets (landscape/portrait/square) or custom W:H ratio (e.g., 3:4, 4:3, 2.35:1) |
| 3 | **Language** | Only if source 鈮?user language | Language for text content |

### Step 5: Generate Prompt 鈫?`prompts/infographic.md`
...
- Named presets 鈫?ratio string: landscape鈫抈16:9`, portrait鈫抈9:16`, square鈫抈1:1`
- Custom W:H ratios 鈫?use as-is (e.g., `3:4`, `4:3`, `2.35:1`)
...
2. Ensure the full final prompt is persisted at `prompts/infographic.md` (already written in Step 5) BEFORE invoking the backend 鈥?the file is the reproducibility record.
